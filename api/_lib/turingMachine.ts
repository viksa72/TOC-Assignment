...
export class TuringMachine {
  ...
  /** Allow injecting snapshot state for stateless step endpoint */
  injectSnapshot(tape: string[], head: number, state: string): void {
    this.tape  = [...tape];
    this.head  = head;
    this.state = state;
  }
  ...
  step(): StepLog | null {
    ...
    if (!action) {
      const log: StepLog = {
        step: this.stepCount, state: this.state, head: this.head,
        read: symbol, written: '—', move: '—', nextState: 'reject',
      };
      this.state = 'reject';
      return log;
    }
    ...
    const log: StepLog = {
      step: this.stepCount, state: this.state, head: this.head,
      read: symbol, written: action.write, move: moveStr, nextState: action.next,
    };
  ...
  run(maxSteps = 1000): { logs: StepLog[]; result: SimResult } {
    const logs: StepLog[] = [];
    ...
    return {
      logs,
      result: exceeded ? 'max_steps' : (this.getResult() ?? 'halted'),
    };
  }
}
